...
class(songs_train_completeDF$counted_stream)

table(songs_test_completeDF$counted_stream)

barplot(prop.table(table(songs_test_completeDF$counted_stream)),
        col = rainbow(2),
...
under <- ovun.sample(counted_stream ~ . , data = songs_train, method = "under", N = 4716)$data
...
confusionMatrix(predict(rf_fit_underSample, songs_test), songs_test$counted_stream)

#confusionMatrix(predict(rf_fit_both, songs_test_completeDF), songs_test_completeDF$counted_stream)
